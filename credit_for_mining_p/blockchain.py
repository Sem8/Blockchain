...
class Blockchain(object):
    ...
    def create_genesis_block(self):
        ''' create the genesis block and add it to the cahin '''
    ...
    def add_block(self, block):
        '''
        Add a received block to end of blockchain

        param block: The validated block sent by another node in the network
        '''
    ...
    def resolve_conflicts(self):
        ...
        our chain witht the longest one in the network.
...
def mine():
    ...
    if blockchain.valid_proof(last_block_string, submitted_proof):
        ...
        # We must receive a reward for finding the submitted_proof.    
        # The sender is "0" to signify that this node has mine a new coin
        # The recipient is the current node, it did the mining!
        # The amount is 1 coin as a reward for mining the next block
        blockchain.new_transaction(id, node_identifier, 1)
...
def last_block():
    # block = json.dumps(blockchain.last_block, sort_keys=True).encode()
    response = {
...
def register_nodes():
    ...
    response = {
        ...
        'total_noded': list(blockchain.nodes)
    }
    return jsonify(response), 200
...
def receive_block():
    ...
    # check index of the block and make sure it is on greater than our last
    ...
    if new_block['index'] == old_block['index'] + 1:
        ...
        # Check their lengths and replace ours with the longes valid chain
        pass
